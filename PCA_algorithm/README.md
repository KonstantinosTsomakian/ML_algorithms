...
For this implementation two approaches are followed:
...
* $U$ is left singular matrix with dimensions $mχm$ and the columns form an orthonormal basis.
* $Σ$ is the singular value matrix which is a diagonal matrix with the singular values on the diagonal.
* V is the right singular matrix which is a $nχn$ matrix where its columns form an orthonormal basis.
